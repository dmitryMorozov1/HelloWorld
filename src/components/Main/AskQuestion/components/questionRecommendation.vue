<template>
  <v-card class="px-4 ask-question-recommendation-comp">
    <v-flex class="pt-3 text-xs-center reg-18 textblue-text">
      Порекомендовать вопрос
    </v-flex>
    <search 
      :element="'ask-question-rec-person-tile-title'"
      :label="'Введите имя'"
      :searchStr="search"   
    ...
    <v-container class="pa-0 scroll-y people-list border">
      <v-list class="py-0">
        <v-list-tile 
          ...
          <v-list-tile-content>
            <div class="ask-question-rec-person-tile-title med-16">
              {{ person.name }}
            </div>
          </v-list-tile-content>
        </v-list-tile>
...
export default {
  ...
  // dialogIsOpen - when you open component from parent repeatedly
  // dialogIsNew - when ask-question-component was close
  props: ['dialogIsOpen', 'dialogIsNew'],
  watch: {
  ...
  methods: {
    ...
    coloringSearch() {
      let peopleNames = document.getElementsByClassName(
                        "ask-question-rec-person-tile-title"
                        );
...
<style scoped>
.ask-question-recommendation-comp {
